...
(function(root,factory){
		...
		validateDom.each(function(item){
			...
			if (requiredArray.indexOf($(this).attr('name'))>=0) {
				...
				if (self.validateClass[index][$(this).attr('name')].group&&!groupArray[self.validateClass[index][$(this).attr('name')].group].ok) {
					...
					for(var k in self.validateClass[index]){
						if (!groupArray[self.validateClass[index][k].group].ok&&k!==$(this).attr('name')&&self.validateClass[index][k].group === self.validateClass[index][$(this).attr('name')].group) {

							groupValidate = $(this).val()?true:false;
							if (!groupValidate) {
	...
	*/
	Lm.prototype.lmGetUserInfo = function(callback){
		...
		if (!self.lmHasStorage()) {
			self.ajax({
				...
				self.lmSetStorage(data.data);
				callback(data.data);
			});
		}else{
			callback(self.lmGetStorage());
		}
	...
	/*
		缓存信息
	*/
	Lm.prototype.lmSetStorage = function(data){
	...
	/*
		获取缓存信息
	*/
	Lm.prototype.lmGetStorage = function(data){

		this.options.userinfo = JSON.parse(sessionStorage.getItem("userinfo"));
	...
	* 获取浏览器url
    */
    Lm.prototype.getPathName = function() {
        // body...
        var pathName = [];
        var params = null ;
